...
El objetivo principal del MT02 consistió en la práctica experimental de conocidas y nuevas herramientas técnicas de tecnología digital, para el diseño modelado 2D y 3D. **Centré mi práctica en las herramientas de modelado 3D desconocidas, para profundizar y entrenar nuevas habilidades técnicas en este campo inmenso orientadas al 3D maker**.  La práctica del MT02, requirió la instalación y seteo de alguna de ellas (ej; Fusion 360) con una modalidad de interfase de uso mixta (local y en la nube), mientras que otras simplemente de uso directo online (ej: Tinkercad 3D).
